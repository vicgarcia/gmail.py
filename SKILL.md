...
Send emails using the `gmail.py` CLI powered by Gmail SMTP. Supports plain text, HTML, attachments, and multiple recipients.
...
# Both from files
gmail.py send --to "recipient@example.com" --subject "Update" \
...
### Quick Message
```bash
gmail.py send --to "friend@example.com" --subject "Quick note" --body "Just wanted to say hi!"
```

### Professional Email with Signature
```bash
gmail.py send --to "client@company.com" --subject "Meeting Follow-up" \
...
### HTML Newsletter
```bash
gmail.py send --to "subscribers@example.com" --subject "Weekly Update" \
...
gmail.py send --to "recipient@example.com" --subject "Detailed Update" \
